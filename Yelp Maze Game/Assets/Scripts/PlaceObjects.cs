...
public class PlaceObjects : MonoBehaviour {
    ...
    public void Place() {
        ...
        for (int o = 0; o < TerrainController.PlaceableObjects.Length; o++){
            ...
            int numObjects = Random.Range(TerrainController.PlaceableObjects[o].min, TerrainController.PlaceableObjects[o].max+1);
            for (int i = 0; i < numObjects; i++){
                int prefabType = o;
                ...
                if (Physics.Raycast(startPoint, Vector3.down, out hit) && hit.point.y > TerrainController.Water.transform.position.y && hit.collider.CompareTag("Terrain")) {
                    ...
                    if (Physics.BoxCast(startPoint, TerrainController.PlaceableObjects[prefabType].size, Vector3.down, out boxHit, orientation) && boxHit.collider.CompareTag("Terrain")) {
                        Instantiate(TerrainController.PlaceableObjects[prefabType].prefab, new Vector3(startPoint.x, hit.point.y, startPoint.z), orientation, transform);
                    }
